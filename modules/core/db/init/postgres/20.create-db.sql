...
-- end EVENTS_STUDENT
-- begin EVENTS_EVENT
alter table EVENTS_EVENT add constraint FK_EVENTS_EVENT_CHIEF1 foreign key (CHIEF_ID) references EVENTS_STUDENT(ID)^
create index IDX_EVENTS_EVENT_CHIEF on EVENTS_EVENT (CHIEF_ID)^
-- end EVENTS_EVENT
-- begin EVENTS_TASK
alter table EVENTS_TASK add constraint FK_EVENTS_TASK_EVENT1 foreign key (EVENT_ID) references EVENTS_EVENT(ID)^
...
-- begin SEC_USER
alter table SEC_USER add constraint FK_SEC_USER_STUDENT1 foreign key (STUDENT_ID) references EVENTS_STUDENT(ID)^
-- end SEC_USER
